...
fn test_axum_end_to_end_generation() {
    ...
    assert!(
        detection.frameworks.contains(&openapi_from_source::cli::Framework::Axum),
...
fn test_actix_end_to_end_generation() {
    ...
    let detection = FrameworkDetector::detect(&parsed_files);
    assert!(
        detection.frameworks.contains(&openapi_from_source::cli::Framework::ActixWeb),
...
fn test_request_body_extraction() {
    ...
    let post_route = routes.iter().find(|r| {
        ...
        matches!(r.method, openapi_from_source::extractor::HttpMethod::Post)
    });
...
fn test_multiple_http_methods_same_path() {
    ...
    if users_routes.len() > 1 {
        ...
        let has_get = methods.iter().any(|m| matches!(m, openapi_from_source::extractor::HttpMethod::Get));
        let has_post = methods.iter().any(|m| matches!(m, openapi_from_source::extractor::HttpMethod::Post));
...
fn test_response_type_extraction() {
    ...
    let get_users_route = routes.iter().find(|r| {
        r.path == "/users" && 
        matches!(r.method, openapi_from_source::extractor::HttpMethod::Get) &&
